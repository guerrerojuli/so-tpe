...
command mvar_cmd = {
    ...
    "MVAR(1)                     User Commands                     MVAR(1)\n\n"
    "NAME\n"
    "       mvar - multiple readers and writers with MVar synchronization\n\n"
    "SYNOPSIS\n"
    "       mvar <num_writers> <num_readers>\n\n"
    "DESCRIPTION\n"
    "       Implements the multiple readers/writers problem using an MVar\n"
    "       (Mutable Variable) pattern similar to Haskell's MVar.\n\n"
    "       Each writer performs random busy waiting, waits for the MVar\n"
    "       to be empty, then writes a unique letter (A, B, C, etc.).\n\n"
    "       Each reader performs random busy waiting, waits for the MVar\n"
    "       to contain a value, then reads and consumes it, printing the\n"
    "       value with a unique identifier.\n\n"
    "       The MVar ensures that:\n"
    "       - Only one process accesses the variable at a time\n"
    "       - Writers can only write when the MVar is empty\n"
    "       - Readers can only read when the MVar is full\n"
    "       - Reading consumes the value (makes it empty)\n\n"
    "       The parent process exits immediately after spawning all\n"
    "       reader and writer processes.\n\n"
    "PARAMETERS\n"
    "       num_writers    Number of writer processes (max 26, for A-Z)\n"
    "       num_readers    Number of reader processes\n\n"
    "EXAMPLES\n"
    "       mvar 2 2       # 2 writers (A,B) and 2 readers\n"
    "       mvar 2 3       # 2 writers (A,B) and 3 readers\n"
    "       mvar 3 2       # 3 writers (A,B,C) and 2 readers\n\n"
    "NOTES\n"
    "       - Writers and readers run indefinitely in background\n"
    "       - Use 'ps' to see created processes\n"
    "       - Use 'kill <pid>' to terminate individual processes\n"
    "       - Use 'nice <pid> <priority>' to change process priority\n"
    "       - Test different scenarios like killing writers/readers\n"
    "       - Test priority changes to see scheduling effects\n\n"};
