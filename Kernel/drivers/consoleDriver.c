...
/**
 * Advances render position based on character type
 */
static void advance_render_position_for_char(char c, render_position_t* pos) {
...
/**
 * Calculates render position for a range in the buffer
 */
static void calculate_position_in_buffer_range(uint64_t start_pos, uint64_t end_pos, render_position_t* pos) {
    ...
    // Calculate position up to start_pos
    for (uint64_t p = visible_start; p < start_pos && p < buffer_length && pos->current_line < lines_per_screen; p++) {
        advance_render_position_for_char(console_buffer[p].c, pos);
    }
}

/**
 * Renders a single character at the current render position
 */
static void render_character_at_position(char c, uint64_t color, render_position_t* pos) {
    uint64_t font_width = get_font_width();
    uint64_t font_height = get_font_height();
    uint64_t screen_width = get_screen_width_pixels();
    ...
    if (c == NEWLINE_CHAR) {
        ...
        // Clear current character area for tab
        draw_rect(DEFAULT_BACKGROUND_COLOR, pos->screen_x, pos->screen_y, 
                 font_width, font_height);
    } else {
        // Clear character area and render if needed
        draw_rect(DEFAULT_BACKGROUND_COLOR, pos->screen_x, pos->screen_y, 
                 font_width, font_height);
    ...
    advance_render_position_for_char(c, pos);
}
 ...
 */
static void console_get_screen_position_from_buffer_pos(uint64_t buffer_pos, uint64_t* screen_x, uint64_t* screen_y) {
    render_position_t pos;
    calculate_position_in_buffer_range(buffer_pos, buffer_pos, &pos);
...
void console_set_font_size(uint64_t font_size) {
    set_font_size(font_size);
    
    // Re-render con las nuevas dimensiones
    last_visible_start = console_calculate_visible_start();
    console_render_all();
}
...
uint64_t console_get_width(void) {
    uint64_t chars_per_line = get_chars_per_line();
    return (chars_per_line > CONSOLE_WIDTH) ? CONSOLE_WIDTH : chars_per_line;
}
...
uint64_t console_get_cursor_x(void) {
    uint64_t current_x = 0;
    uint64_t visible_start = console_calculate_visible_start();
    uint64_t chars_per_line = get_chars_per_line();
    
    for (uint64_t pos = visible_start; pos < cursor_position && pos < buffer_length; pos++) {
        char c = console_buffer[pos].c;
...
uint64_t console_get_cursor_y(void) {
    uint64_t current_line = 0;
    uint64_t current_x = 0;
    uint64_t visible_start = console_calculate_visible_start();
    uint64_t chars_per_line = get_chars_per_line();
...
void console_set_cursor(uint64_t x, uint64_t y) {
    // Por ahora, implementación simple - mueve cursor al final
    cursor_position = buffer_length;
}

void console_rerender(void) {
    console_render_all();
}

void console_flush(void) {
    // No hace nada porque ya no tenemos batching
