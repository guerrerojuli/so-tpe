...
# Usage:
#   ./compile_buddy.sh            # build inside Docker (BUDDY)
#   ./compile_buddy.sh PVS        # build and run PVS (requires so-pvs image)
...
echo "Building with memory manager: BUDDY (Docker image: ${IMAGE})"
...
if [ ${RUN_PVS} -eq 1 ]; then
	...
	docker exec -it "${CONTAINER}" bash -lc "
		...
		make clean -C ./
		export MAKEFLAGS=
		pvs-studio-analyzer trace -- sh -c 'make -C ./Toolchain && make -C ./ MM=BUDDY all'
